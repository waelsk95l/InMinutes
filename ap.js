const url = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=PLmrET10kAE97BDUhRyter_fXXvExizuUs&maxResults=50&key=AIzaSyAaRjH-yWMxGLPBOvDdqkGjTn84-wqY--o";
fetch(url).then( response => { return response.json()}).then(data =>{

        getData(data.items);
        document.getElementById("player").src= "https://www.youtube.com/embed/"+ data.items[11].snippet.resourceId.videoId+ "?showinfo=0; rel=0;";
...
setInterval(function () {
    document.getElementById("player").style.height =document.getElementById("player").getBoundingClientRect().width * 0.5625 + "px";
